...
import org.webdriver.patatiumappui.utils.ExcelUtils;
...
public class PageObjectAutoCodeForYaml {
    ...
    //根据excel生成pageObject文件
    public static void autoCode3(String path) throws Exception {
        ExcelUtils excelUtils = new ExcelUtils(path);
        excelUtils.getSheetData();
        //获取要写入的page所属的类名
        String pageClassName = excelUtils.getCellData(1, 2);
        String pageName = "org.webdriver.patatiumappui.pageObject." + pageClassName;
        String[] pageNameArray = pageName.split("\\.");
        //获取对象库包名
        String packageName = pageNameArray[0].toString() + "." + pageNameArray[1].toString() + "." + pageNameArray[2].toString() + "." + pageNameArray[3].toString();

        String value = excelUtils.getCellData(2, 2);
        String desc = excelUtils.getCellData(3, 2);

        List<List<String>> sheet = excelUtils.getListData(6);
        ...
        //对象库xml文件根节点
        //遍历根节点下的第一个节点（page节点）
        for (Iterator<?> i = sheet.iterator(); i.hasNext(); ) {
            List<String> locator = (List<String>) i.next();

            //sb.append("\n private String path=PageObjectAutoCode.class.getClassLoader().getResource(\"net/hk515/pageObjectConfig/UILibrary.xml\").getPath();");


            String locatorName = locator.get(4);
            if (!locator.get(3).isEmpty()) {
                sb.append("\n/***\n"
                        + "* " + locator.get(2) + "\n"
                        + "* @return\n"
                        + "* @throws IOException\n"
    ...
    public static void main(String[] args) throws Exception {
        // TODO 自动生成的方法存根
        PageObjectAutoCodeForYaml.autoCode3("F:\\UITest\\page.xlsx");
    }
